...
가입 흐름을 태우면 실패 원인이 앱인지 문자인지 가리기 어렵다.
  ...
  python scripts/send_test_sms.py 01012345678 --message "테스트입니다"
...
from app.services.sms import SMSError, send_verification_sms


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("phone", help="받는 번호. 하이픈은 있어도 된다")
    ...
    if settings.sms_provider == "mock":
        ...
        print("   실제 발송을 보려면 SMS_PROVIDER 를 aligo 나 ncp 로 두고 다시 실행할 것.")
        print()
